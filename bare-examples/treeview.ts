...
class TreeViewExample {
  ...
  private listStore?: Gtk.ListStore;
  private treeView?: Gtk.TreeView;
  ...
  private buildUI() {
    ...
    let firstName = new Gtk.TreeViewColumn({
      title: "First Name",
      resizable: true,
    });
    let lastName = new Gtk.TreeViewColumn({
      title: "Last Name",
      resizable: true,
    });
    let phone = new Gtk.TreeViewColumn({
      title: "Phone Number",
      resizable: true,
    });
    ...
    this.treeView.insert_column(firstName, 0);
    this.treeView.insert_column(lastName, 1);
    this.treeView.insert_column(phone, 2);
    ...
    this.selection.connect("changed", this.onSelectionChanged.bind(this));
  ...
  private onSelectionChanged() {
    ...
    this.label.set_label(
      "\n" +
        ...
        "\n" +
        this.listStore?.get_value(iter, 3)
    );
  }
}

const app = new TreeViewExample();
//@ts-ignore
app.run(ARGV);
